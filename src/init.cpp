...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/traviscoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in traviscoind / RPC client
            std::string strUsage = _("TravisCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  traviscoind [options]                     " + "\n" +
                  "  traviscoind [options] <command> [params]  " + _("Send command to server or traviscoind") + "\n" +
                  "  traviscoind [options] help                " + _("List commands") + "\n" +
                  "  traviscoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "traviscoin:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("TravisCoin"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("TravisCoin"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: traviscoin.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: traviscoind.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // traviscoin: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // traviscoin: checkpoint master priv key
    {
    ...
    // Make sure only a single TravisCoin process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  TravisCoin is probably already running."), pszDataDir));
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("TravisCoin"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
